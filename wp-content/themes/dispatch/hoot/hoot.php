...
if ( !class_exists( 'Hoot' ) ) {
	class Hoot {
		...
		function i18n() {
			...
			$this->textdomain_loaded[ $parent_textdomain ] = load_theme_textdomain( $parent_textdomain, get_template_directory() . '/languages' );

			/* Load child theme textdomain. */
			$this->textdomain_loaded[ $child_textdomain ] = is_child_theme() ? load_child_theme_textdomain( $child_textdomain ) : false;

			/* Load the framework textdomain. */
			// @disabled: WordPress standards allow only 1 text domain (theme slug), so we will stick to that.
			// $this->textdomain_loaded['dispatch'] = hoot_load_framework_textdomain( 'dispatch' );
			// $this->textdomain_loaded['dispatch'] = hoot_load_framework_textdomain( 'dispatch' );
