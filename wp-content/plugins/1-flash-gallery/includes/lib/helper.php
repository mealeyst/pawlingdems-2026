...
//    return sprintf('[fgallery id=%d w=%d h=%d tbg=%d t=%d title="%s"]',$id, 
//            $album['gall_width'], $album['gall_height'], /*$album['gall_transparentbg'],*/ $type, trim($album['gall_name']));
//}
...
function fgallery_search_flash_path($album) {
    ...
    if (get_option('1_flash_gallery_'.fgallery_get_flash_type($album)) !='') {
        if (plugin_is_active('1-flash-gallery-update-'.fgallery_get_flash_type($album).'/update_'.fgallery_get_flash_type($album).'.php')) {
                return WP_PLUGIN_URL .'/1-flash-gallery-update-'.fgallery_get_flash_type($album).'/swf/'.fgallery_get_flash_type($album).'.swf';
        } elseif (plugin_is_active('1-flash-gallery-update/update_fgallery.php')) {
