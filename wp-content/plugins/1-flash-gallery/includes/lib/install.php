...
function fgallery_first_install() {
   ...
   if($wpdb->get_var("SHOW TABLES LIKE '" . ALBUMS_TABLE . "'") != ALBUMS_TABLE) {
	...
	$create_albums = "CREATE TABLE " . ALBUMS_TABLE . " (
                            ...
                            `gall_transparentbg` tinyint(1) default 0,
...
function fgallery_update($fi = false) {
    ...
    if (version_compare($version, '1.8.3','<')) {
        add_option('1_flash_gallery_access_level', 'manage_options','','no');
    }
    ...
    if (version_compare($version, '1.9.2','<')) {
        if(!$fi)$S=$S&$wpdb->query("ALTER TABLE `".ALBUMS_TABLE."` ADD `gall_transparentbg` BOOLEAN NULL DEFAULT '0' AFTER `gall_bgcolor`;");
    }
...
function fgallery_create_folders() {
  if ( ! is_dir( FGALLERY_DIR ) ) {
    wp_mkdir_p( FGALLERY_DIR . '/tmp' );
  }
}
