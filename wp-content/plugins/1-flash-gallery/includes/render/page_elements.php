 ...
 * @return string 
 */
function fgallery_render_edit_album_form($album, $available_galleries) {
    $id = $album['gall_id'];
...
//Dropaq: hide till new updte
//    $output .= '<div class="form-item even">
//                    <label for="fgalleryTransparent">'.__('Transparent Background:', 'fgallery').'</label>
//                    <input type="checkbox" id="fgalleryBgcolor" name="gallery[gall_transparentbg]" 
//                    class="form-checkbox" value="1" '.(($album['gall_transparentbg'])?'checked="checked"':'').' />                    
//                </div>';
    ...
    $output .= '<div class="form-item odd">
        ...
        if(isset($available_galleries[1])) $output .= '<option value="1" '.@$opt[1].'>'.__('Acosta').'</option>';
        if(isset($available_galleries[2])) $output .= '<option value="2" '.@$opt[2].'>'.__('Airion').'</option>';
        if(isset($available_galleries[3])) $output .= '<option value="3" '.@$opt[3].'>'.__('Arai').'</option>';
        if(isset($available_galleries[4])) $output .= '<option value="4" '.@$opt[4].'>'.__('Pax').'</option>';
        if(isset($available_galleries[5])) $output .= '<option value="5" '.@$opt[5].'>'.__('Pazin').'</option>';
        if(isset($available_galleries[6])) $output .= '<option value="6" '.@$opt[6].'>'.__('Postma').'</option>';
        if(isset($available_galleries[7])) $output .= '<option value="7" '.@$opt[7].'>'.__('PageFlip').'</option>';
        if(isset($available_galleries[8])) $output .= '<option value="8" '.@$opt[8].'>'.__('Nilus').'</option>';
        if(isset($available_galleries[9])) $output .= '<option value="9" '.@$opt[9].'>'.__('Nusl').'</option>';
        if(isset($available_galleries[10])) $output .= '<option value="10" '.@$opt[10].'>'.__('Kranjk').'</option>';
        if(isset($available_galleries[11])) $output .= '<option value="11" '.@$opt[11].'>'.__('Perona').'</option>';
        if(isset($available_galleries[12])) $output .= '<option value="12" '.@$opt[12].'>'.__('Ables').'</option>';
        if(isset($available_galleries[13])) $output .= '<option value="13" '.@$opt[13].'>'.__('PageFlip 2.0').'</option>';
