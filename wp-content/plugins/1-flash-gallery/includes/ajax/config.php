...
//var_dump($gallery_type, $current_group);
                    if ($gallery_type == 'pageflip' && $current_group == 'menu'){
                        $srcAttribute = $configXML->createAttribute('src');
                        $srcAttribute->value = get_option('siteurl').'/wp-content/plugins/1-flash-gallery/swf/skin/'.$matches[2].'.swf';
