...
function fgallery_publ() {
    ...
    if ($album['gall_type'] == 13) {
        ...
        header("Content-type: text/xml");
        fgallery_build_publXML($album, $items);
    } else {
 ...
 * @param array $items 
 */
function fgallery_build_publXML($album, $items) {
    // create new XML document
...
$null_item[0] = array();
$null_item[0]['img_id'] = $album['gall_id'];
$null_item[0]['gall_cover'] = $album['gall_cover'];
$null_item[0]['gall_name'] = $album['gall_name'];
//var_dump($items);
$items = array_merge($null_item, $items);
//var_dump($items);
    // create items
    if (!empty($items)) {
        foreach ($items as $item) {
            ...
            if (isset($item['gall_cover']) && $item['gall_cover'] != 0){
...
function fgallery_pf_style(){
        
    $file_css = @fopen(FGALLERY_ABSPATH.'/swf/style.css', 'r');
    
    $output_style = '';
    
    while ($line = fgets($file_css, 4096)){
    
        $output_style .= str_replace('FGALLERY_PATH', FGALLERY_PATH, $line);
