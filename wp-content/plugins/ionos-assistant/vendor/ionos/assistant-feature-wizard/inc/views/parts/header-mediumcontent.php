<?php
/**
 * @global $args
 */

use Ionos\Assistant\Config;

?>
<div class="wrapper">
	<div class="header">
		<img class="logo" src="<?php echo Config::get( 'branding.logo_variant1' ); ?>" />
	</div>
	<div class="container medium">
		<p class="counter"><?php echo ! empty( $args['counter_text'] ) ? $args['counter_text'] : __( 'Counter missing', 'ionos-assistant' ); ?></p>
		<h1 class="headline"><?php echo ! empty( $args['heading_text'] ) ? $args['heading_text'] : __( 'HEADING MISSING', 'ionos-assistant' ); ?></h1>
		<form method="get">
			<input type="hidden" name="page" value="ionos-assistant">
			<input type="hidden" name="step" value="<?php echo ! empty( $args['next_step'] ) ? $args['next_step'] : ''; ?>">
			<div class="content">
